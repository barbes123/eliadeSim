	...
	if (currentMerge > lastMerge && doMerge)
	{
		...
		for (unsigned int i = 0; i < AnalysisObjects.size(); i++)
		{
			...
			for (unsigned int o = 0; o < AnalysisObjects[i].size(); o++)
			{
				...
				//	long long int firstTime = 0;
				//	long long int secondTime = 0;
				...
				//	theSize	= AnalysisObjects[i][0]->digitalEvents.size();
				//	firstTime = get<0>(AnalysisObjects[i][0]->digitalEvents[0]);
				//	secondTime = get<0>(AnalysisObjects[i][0]->digitalEvents.back());

				//	G4double theRate = (theSize*1.0*0.001)/((secondTime-firstTime)*pow(10,-9));

				//	AnalysisObjects[i][0]->AddRate(theRate);

				AnalysisObjects[i][o]->RecordDigitalEvents();

				/*
				for(unsigned int j = 1; j < AnalysisObjects[i].size(); j++){

					AnalysisObjects[i][j]->AddAddBackEvents(AnalysisObjects[i][0]->GetAddBackEvents());

					if(AnalysisObjects[i][j]->digitalEvents.size() != 0){

						AnalysisObjects[i][j]->RecordAddBackEvents();

					}

				}

				AnalysisObjects[i][0]->ClearAddBackEvents();*/
			}
		}
		...
		if (FInput->GetGeometryObjects()[i].GetObjectActiveStatus())
		{
			...
			if (objects[i].GetObjectType() == "PIXEL")
			{
				...
				/*
							...
							detanalysis->SetObjectId(i);				// not properly set

							for(unsigned int o = 0; o < objects[i].GetObjectActiveVolumes().size(); o++){



								pixAnalysis->SetObjectId(i*1000+o);		// not properly set
					...
					if (isShielded)
					{
						...
						for (unsigned int o = 0; o < objects[i].GetObjectActiveVolumes().size(); o++)
						{
							...
							else
							{
								...
								if (o % 42 < 40){side=string("Front");}
								if (o % 42 < 36){side=string("Side");}
